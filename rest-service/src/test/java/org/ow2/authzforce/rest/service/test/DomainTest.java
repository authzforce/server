 ...
 * <p>
 * TODO: tests for max number of policies per domain, and max number of versions
 * per policy.
 *
 */
...
public class DomainTest extends AbstractTestNGSpringContextTests
{
	...
	static
	{
		try
		{
			JAXB_CTX = JAXBContext.newInstance(PolicySet.class, Request.class,
					Resources.class, TestAttributeProvider.class);
		} catch (JAXBException e)
		{
			throw new RuntimeException(
					"Error instantiating JAXB context for XML to Java binding",
					e);
		}
	}

	/**
	 * XACML policy filename used by default when no PDP configuration file
	 * found, i.e. no file named {@value #PDP_CONF_FILENAME} exists in the test
	 * directory
	 */
	...
	@BeforeClass
	public void addDomain(ITestContext testCtx) throws JAXBException,
			IOException
	{
		/*
		 * WARNING: if tests are to be multi-threaded, modify according to
		 * Thread-safety section of CXF JAX-RS client API documentation
		 * http://cxf
		...
		final Link domainLink = client.addDomain(new DomainProperties(
				"Some description", EXTERNAL_ID));
		...
		{
			throw new RuntimeException("Test data directory '"
					+ xacmlSamplesDirResource + "' does not exist");
		}
	...
	{
		assertNotNull(testDomain.deleteDomain(),
				String.format("Error deleting domain ID=%s", testDomainId));
	}
	...
	{
		final DomainPropertiesResource propsResource = testDomain
	...
	public void updateDomainProperties()
	{
		...
		String externalId = "external"
				+ FileBasedDAOUtils.base64UrlEncode(randomBytes2);
		propsResource.updateDomainProperties(new DomainProperties(description,
				externalId));
		// verify result
		final DomainProperties newProps = testDomain
				.getDomainPropertiesResource().getDomainProperties();
	...
	public void getPap()
	{
		final ResourceContent papResource = testDomain.getPapResource()
				.getPAP();
	...
	{
		final AttributeProviders attributeProviders = testDomain
				.getPapResource().getAttributeProvidersResource()
	...
	{
		final AttributeProvidersResource attributeProvidersResource = testDomain
				.getPapResource().getAttributeProvidersResource();
		JAXBElement<TestAttributeProvider> jaxbElt = (JAXBElement<TestAttributeProvider>) JAXB_CTX
				.createUnmarshaller().unmarshal(
						new File(xacmlSamplesDir,
								"pdp/IIA002(PolicySet)/attributeProvider.xml"));
		...
		AttributeProviders updateAttrProvidersResult = attributeProvidersResource
				.updateAttributeProviderList(new AttributeProviders(
						Collections
								.<AbstractAttributeProvider> singletonList(testAttrProvider)));
		assertNotNull(updateAttrProvidersResult);
		assertEquals(updateAttrProvidersResult.getAttributeProviders().size(),
				1);
		AbstractAttributeProvider updateAttrProvidersItem = updateAttrProvidersResult
				.getAttributeProviders().get(0);
		if (updateAttrProvidersItem instanceof TestAttributeProvider)
		{
			assertEquals(
					((TestAttributeProvider) updateAttrProvidersItem)
							.getAttributes(),
		...
		AttributeProviders getAttrProvidersResult = attributeProvidersResource
				.getAttributeProviderList();
		assertNotNull(getAttrProvidersResult);
		assertEquals(getAttrProvidersResult.getAttributeProviders().size(), 1);
		AbstractAttributeProvider getAttrProvidersItem = getAttrProvidersResult
				.getAttributeProviders().get(0);
		if (getAttrProvidersItem instanceof TestAttributeProvider)
		{
			assertEquals(
					((TestAttributeProvider) getAttrProvidersItem)
							.getAttributes(),
					testAttrProvider.getAttributes());
		} else
		{
	...
	{
		final Resources resources = testDomain.getPapResource()
				.getPoliciesResource().getPolicies();
		assertNotNull(resources);
		assertTrue(resources.getLinks().size() > 0,
				"No resource for root policy found");
	}

	@Test(dependsOnMethods = { "getDomainProperties", "getPolicies" })
	public void getRootPolicy()
	{
		final IdReferenceType rootPolicyRef = testDomain.getPapResource()
				.getPdpPropertiesResource().getPdpProperties()
				.getRootPolicyRef();
		final List<Link> links = testDomain.getPapResource()
				.getPoliciesResource()
	...
	private static void matchPolicySets(PolicySet actual, PolicySet expected,
			String testedMethodId)
	{
		assertEquals(
				actual.getPolicySetId(),
				expected.getPolicySetId(),
				String.format(
						"Actual PolicySetId (='%s') from %s() != expected PolicySetId (='%s')",
						actual.getPolicySetId(), testedMethodId,
						expected.getPolicySetId()));
		assertEquals(
				actual.getVersion(),
				expected.getVersion(),
				String.format(
						...
						actual.getVersion(), testedMethodId,
						expected.getVersion()));
	}
	...
	/**
	 * Adds a given PolicySet and returns allocated resource ID. It also checks
	 * 2 things: 1) The response's PolicySet matches the input PolicySet (simply
	 * checking PolicySet IDs and versions) 2) The reponse to a getPolicySet()
	 * also matches the input PolicySet, to make sure the new PolicySet was
	 * actually committed succesfully and no error occurred in the process. 3)
	 * Response to getPolicyResources() with same PolicySetId and Version
	 * matches.
	 * <p>
	...
	{
		// put new policyset
		final Link link = testDomain.getPapResource().getPoliciesResource()
				.addPolicy(policySet);
		final String policyHref = link.getHref();
		final String[] parts = policyHref.split("/");
		assertTrue(parts.length >= 2,
				"Link returned by addPolicy does not match pattern 'policyId/version'");
		...
		PoliciesResource policiesRes = testDomain.getPapResource()
				.getPoliciesResource();
		assertTrue(
				isHrefMatched(policyResId, policiesRes.getPolicies().getLinks()),
		...
		assertTrue(
				isHrefMatched(versionResId, policyVersionsResources.getLinks()),
		...
		// added
		final PolicySet getRespPolicySet = policyRes.getPolicyVersionResource(
				versionResId).getPolicyVersion();
	...
	public void addAndGetPolicy()
	{
		...
		Resources policyVersionsResources = testDomain.getPapResource()
				.getPoliciesResource().getPolicyResource("testPolicyAdd")
				.getPolicyVersions();
	...
	{
		PolicySet policySet = createDumbPolicySet(
				"testAddConflictingPolicyVersion", "1.0");
		testAddAndGetPolicy(policySet);
	...
	public void deletedAndTryGetUnusedPolicy()
	{
		...
		PolicyResource policyResource = testDomain.getPapResource()
				.getPoliciesResource().getPolicyResource("testPolicyDelete");
		...
		PoliciesResource policiesRes = testDomain.getPapResource()
				.getPoliciesResource();
		assertFalse(
				isHrefMatched("testPolicyDelete", policiesRes.getPolicies()
						.getLinks()),
	...
	public void deletedAndTryGetPolicyVersion()
	{
		...
		PolicyVersionResource policyVersionRes = testDomain.getPapResource()
				.getPoliciesResource().getPolicyResource("testPolicyDelete")
		...
		try
		{
			...
			org.testng.Assert
					.fail("Policy version removal failed (resource still there");
		} catch (NotFoundException e)
		{
			// OK
		}
	...
	public void deleteRootPolicy()
	{
		...
		try
		{
			testDomain.getPapResource().getPoliciesResource()
					.getPolicyResource(rootPolicyRef.getValue()).deletePolicy();
		...
		assertEquals(rootPolicyRef, testDomain.getPapResource()
				.getPdpPropertiesResource().getPdpProperties()
				.getRootPolicyRef(),
	...
	{
		PdpPropertiesResource propsRes = testDomain.getPapResource()
				.getPdpPropertiesResource();
		IdReferenceType newRootPolicyRef = new IdReferenceType(policyId,
				version, null, null);
		propsRes.updateOtherPdpProperties(new PdpPropertiesUpdate(
				newRootPolicyRef));
	...
	@Test(dependsOnMethods = { "addAndGetPolicy", "deleteRootPolicy",
			"updateDomainPropertiesExceptRootPolicyRef" })
	public void updateRootPolicyRefToValidPolicy() throws JAXBException
	{
		...
		final IdReferenceType rootPolicyRef = propsRes.getPdpProperties()
				.getRootPolicyRef();
		PolicyResource oldRootPolicyRes = testDomain.getPapResource()
				.getPoliciesResource()
				.getPolicyResource(rootPolicyRef.getValue());
		...
		// verify update
		final PdpProperties newProps = testDomain.getPapResource()
				.getPdpPropertiesResource().getPdpProperties();
	...
	public void updateRootPolicyRefToMissingPolicy() throws JAXBException
	{
		...
		IdReferenceType newRootPolicyRef = new IdReferenceType(newRootPolicyId,
				null, null, null);
	...
	public void updateRootPolicyRefToValidVersion() throws JAXBException
	{
		...
		PolicySet policySet = createDumbPolicySet(
				"testUpdateRootPolicyRefToValidVersion", "4.4");
		testAddAndGetPolicy(policySet);

		PolicySet policySet2 = createDumbPolicySet(
				"testUpdateRootPolicyRefToValidVersion", "4.5");
		testAddAndGetPolicy(policySet2);

		PolicySet policySet3 = createDumbPolicySet(
				"testUpdateRootPolicyRefToValidVersion", "4.6");
		testAddAndGetPolicy(policySet3);

		IdReferenceType newRootPolicyRef = new IdReferenceType(
				"testUpdateRootPolicyRefToValidVersion", "4.5", null, null);
		propsRes.updateOtherPdpProperties(new PdpPropertiesUpdate(
				newRootPolicyRef));
		...
		PolicyResource policyRes = testDomain.getPapResource()
				.getPoliciesResource()
	...
	public void updateRootPolicyRefToMissingVersion() throws JAXBException
	{
		...
		IdReferenceType newRootPolicyRef = new IdReferenceType(rootPolicyId,
				"0.0.0.1", null, null);
		// MUST FAIL
		try
		{
	...
	{
		final PolicySet rootPolicySet = (PolicySet) unmarshaller
				.unmarshal(new File(xacmlSamplesDir, DEFAULT_POLICYSET_FILENAME));
		final PoliciesResource policiesRes = testDomain.getPapResource()
				.getPoliciesResource();

		// If already exists, remove to start from scratch
		PolicyResource rootPolicyRes = policiesRes
				.getPolicyResource(rootPolicySet.getPolicySetId());
		try
		{
	...
	{
		/*
		 * Before putting the empty refPolicySets (to make a policySets with
		 * references wrong), first put good root policySet without policy
		 * references (in case the current root PolicySet has references,
		 * uploading the empty refPolicySets before will be rejected because it
		 * makes the policySet with references invalid)
		 */
		resetPolicies();

		final PolicySet refPolicySet = (PolicySet) unmarshaller
				.unmarshal(new File(xacmlSamplesDir,
		...
		final PolicySet policySetWithRef = (PolicySet) unmarshaller
				.unmarshal(new File(xacmlSamplesDir,
		...
		final PolicyResource refPolicyResource = testDomain.getPapResource()
				.getPoliciesResource().getPolicyResource(refPolicyResId);
		try
		{
		...
		/*
		 * Check that referenced policy set was not actually deleted, is still
		 * there
		 */
		final PolicySet getPolicyVersion = refPolicyResource
				.getPolicyVersionResource(refPolicySet.getVersion())
				.getPolicyVersion();
	...
	public void setRootPolicyWithRefToMissingPolicy() throws JAXBException
	{
		...
		final PolicySet policySetWithRefs = (PolicySet) unmarshaller
				.unmarshal(new File(xacmlSamplesDir,
	...
	public void setRootPolicyWithCircularRef() throws JAXBException
	{
		...
		final PolicySet refPolicySet2 = (PolicySet) unmarshaller
				.unmarshal(new File(xacmlSamplesDir,
	...
	public void addPolicyWithTooManyChildElements() throws JAXBException
	{
		...
		// Policy(Set)IdReferences do
		// not exist since refPolicySets empty)
		final PolicySet badPolicySet = (PolicySet) unmarshaller
				.unmarshal(new File(xacmlSamplesDir,
		...
		testDomain.getPapResource().getPoliciesResource()
				.getPolicyResource(badPolicySet.getPolicySetId())
				.getPolicyVersions();
	}
	...
	public void addPolicyTooDeep() throws JAXBException
	{
		...
		// not exist since refPolicySets empty)
		final PolicySet badPolicySet = (PolicySet) unmarshaller
				.unmarshal(new File(xacmlSamplesDir, "policySetTooDeep.xml"));
		try
		{
			testAddAndGetPolicy(badPolicySet);
	...
	/**
	 * Create PDP evaluation test data. Various PolicySets/Requests/Responses
	...
	public Iterator<Object[]> createData() throws URISyntaxException,
			IOException
	{
		final Collection<Object[]> testParams = new ArrayList<>();
		/*
		 * Each sub-directory of the root directory is data for a specific test.
		 * So we configure a test for each directory
		 */
	...
	private static void assertNormalizedEquals(Response expectedResponse,
			Response actualResponseFromPDP) throws JAXBException
	{
		// normalize responses for comparison
		final Response normalizedExpectedResponse = TestUtils
				.normalizeForComparison(expectedResponse);
		final Response normalizedActualResponse = TestUtils
				.normalizeForComparison(actualResponseFromPDP);
		assertEquals(
	...
	public void requestPDP(File testDirectory) throws JAXBException
	{
		...
		testDomain
				...
				.updateAttributeProviderList(
						new AttributeProviders(Collections
								.<AbstractAttributeProvider> emptyList()));

		LOGGER.debug("Starting PDP test of directory '{}'", testDirectory);

		final File attributeProviderFile = new File(testDirectory,
				ATTRIBUTE_PROVIDER_FILENAME);
		if (attributeProviderFile.exists())
		{
			...
			testDomain
					...
					.getAttributeProvidersResource()
					.updateAttributeProviderList(
							new AttributeProviders(
									Collections
											.<AbstractAttributeProvider> singletonList(jaxbElt
													.getValue())));
		}

		final File refPoliciesDir = new File(testDirectory,
				REF_POLICIES_DIRECTORY_NAME);
		if (refPoliciesDir.exists() && refPoliciesDir.isDirectory())
		{
			for (final File policyFile : refPoliciesDir.listFiles())
			{
				...
				{
					final PolicySet policy = (PolicySet) unmarshaller
							.unmarshal(policyFile);
		...
		final PolicySet policy = (PolicySet) unmarshaller.unmarshal(new File(
				testDirectory, POLICY_FILENAME));
		// Add the policy and point the rootPolicyRef to new policy with refs to
		// instantiate it as root policy (validate, etc.)
		setRootPolicy(policy);

		final Request xacmlReq = (Request) unmarshaller.unmarshal(new File(
				testDirectory, REQUEST_FILENAME));
		final Response expectedResponse = (Response) unmarshaller
				.unmarshal(new File(testDirectory, EXPECTED_RESPONSE_FILENAME));
		final Response actualResponse = testDomain.getPdpResource()
				.requestPolicyDecision(xacmlReq);
