...
<!-- PDP configuration upgrade XSL Sheet: 4.3.0 -> 5.x and above. To be used with Saxon XSLT processor. Author: Cyril DANGERVILLE. -->
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns="http://authzforce.github.io/core/xmlns/pdp/3.6">
   ...
   <xsl:template match="/*">
      <xsl:element name="{local-name(.)}">
         ...
         <xsl:namespace name="flat-file-dao" select="'http://authzforce.github.io/pap-dao-flat-file/xmlns/pdp-ext/3.6'" />
         <xsl:apply-templates select="@* | node()" />
      </xsl:element>
   </xsl:template>

   <!-- Convert @xsi:type="old-dao:StaticFileBasedDAORefPolicyProvider" to "flat-file-dao:StaticFlatFileDAORefPolicyProvider -->
   <xsl:template match="@xsi:type[resolve-QName(., ..) = QName('http://authzforce.github.io/pap-dao-file/xmlns/pdp-ext/3.6','StaticFileBasedDAORefPolicyProvider')]">
