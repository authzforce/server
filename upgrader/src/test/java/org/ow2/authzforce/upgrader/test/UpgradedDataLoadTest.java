...
public class UpgradedDataLoadTest extends AbstractTestNGSpringContextTests {
	...
	private static final Logger LOGGER = LoggerFactory
			.getLogger(UpgradedDataLoadTest.class);
	...
	private Set<String> createdDomainIds = new HashSet<>(Arrays.asList(
			"5e022256-6d0f-4eb8-aa9d-77db3d4ad141",
			"6e022256-6d0f-4eb8-aa9d-77db3d4ad141"));
	...
	public void beforeSuite(@Optional(DEFAULT_APP_BASE_URL) String appBaseUrl,
		...
		if (startServer) {
			...
			try {
				...
				ic.bind("java:comp/env/org.ow2.authzforce.config.dir", confURI);
	...
	@Test(dependsOnMethods = { "getDomainLinks" })
	public void getDomainResources() {
		for (String createdDomainId : createdDomainIds) {
			Domain testDomainResource = client.getDomainResource(
					createdDomainId).getDomain();
			assertNotNull(testDomainResource, String.format(
					"Error retrieving domain ID=%s", createdDomainId));
		}
	}

	@Test(dependsOnMethods = { "getDomainResources" })
	public void getDomainsByExternalId() {
		for (String createdDomainId : createdDomainIds) {
			String externalId = client.getDomainResource(createdDomainId)
					.getDomain().getProperties().getExternalId();

			final List<Link> domainLinks = client.getDomains(externalId)
					.getLinks();
			// verify that there is only one domain resource link and it is the
			// one
			// we are looking for
	...
	@Test(dependsOnMethods = { "getDomainsByExternalId" })
