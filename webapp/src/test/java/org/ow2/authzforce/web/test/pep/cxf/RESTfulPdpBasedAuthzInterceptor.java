...
public class RESTfulPdpBasedAuthzInterceptor extends AbstractPhaseInterceptor<Message>
{
	...
	private static Request createRequest(final Principal principal, final Set<String> roles, final Message message) throws WSSecurityException
	{
		...
		final Attributes subjectCategory = new Attributes(null, Arrays.asList(subjectIdAtt, subjectRoleAtt), XACML_1_0_ACCESS_SUBJECT.value(), null);
		...
		final Attributes resourceCategory = new Attributes(null, resourceAtts, XACML_3_0_RESOURCE.value(), null);
		...
		final Attributes actionCategory = new Attributes(null, Collections.singletonList(actionIdAtt), XACML_3_0_ACTION.value(), null);
